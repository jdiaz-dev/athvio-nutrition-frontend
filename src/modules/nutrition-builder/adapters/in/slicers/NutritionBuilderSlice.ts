...
const nutritionBuilderSlice = createSlice({
  ...
  reducers: {
    ...
    updateDiseaseCause(state, action: PayloadAction<{ uuid: string; status: NutriBuilderParamStatus }>) {
      const index = state.diseaseCauses.findIndex((item) => item.uuid === action.payload.uuid);
      if (index !== -1) state.diseaseCauses[index].status = action.payload.status;
      return state;
    },
    updateNutritionalPreference(state, action: PayloadAction<{ uuid: string; status: NutriBuilderParamStatus }>) {
      const index = state.nutritionalPreferences.findIndex((item) => item.uuid === action.payload.uuid);
      if (index !== -1) state.nutritionalPreferences[index].status = action.payload.status;
      return state;
    },
    updateDisease(state, action: PayloadAction<{ uuid: string; status: NutriBuilderParamStatus }>) {
      const index = state.diseases.findIndex((item) => item.uuid === action.payload.uuid);
